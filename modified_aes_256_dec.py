...
def key_whitening(block_grid, key_grid):
    r = []
...
def extract_key_for_round(expanded_key, round):
    return [row[round*4: round*4 + 8] for row in expanded_key]
...
def break_in_grids_of_32(s): #forming grids of 4*8 size(256 bit total size of a grid)
    all_grids = []
    for i in range(len(s) // 32):  # Each grid is 32 bytes (256 bits)
        block = s[i * 32: i * 32 + 32]
        grid = [[], [], [], []]  # 4x8 array
        for i in range(8):  # Iterate over each column
            for j in range(4):  # Iterate over each row
                grid[j].append(block[i + j * 8])
...
def expand_key(key, rounds):
    ...
    key_grid = break_in_grids_of_32(key)[0]
    ...
    for round in range(rounds):
        ...
        for r in range(4):
            result = last_column_rcon_step[r] ^ key_grid[r][round*8]
            result &= 0xff
            key_grid[r] += bytes([result])

        #7 more columns to go more columns to go
        for i in range(len(key_grid)):
            for j in range(1, 8):
                key_grid[i] += bytes([key_grid[i][round*8+j]^ key_grid[i][round*4+j+7]])
...
def encryption(key, data):
    ...
    expanded_key = expand_key(key, 15)
    ...
    temp_grids = []
    round_key = extract_key_for_round(expanded_key, 14)
...
if __name__ == "__main__":
    ...
    #key= generate_aes_key(256) #256 bit key
    key=b'190345bfrthncde045%@aked$5!2@0ml'
    
    # Decrypting the data
    filetype=input("Enter type of file to decrypt:")
    a,ext=filetype.split('.')
    with open('cyphertext'+'.'+ext, 'rb') as f:
       cyphertext = f.read()
        
    decrypted_data = decryption(key, cyphertext)

    with open('decrypted'+'.'+ext, 'wb') as f:
